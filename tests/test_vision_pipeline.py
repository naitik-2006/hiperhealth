...
from hiperhealth.vision.pipeline import MedVisionPipeline, VisionExtractionResult, VisionFinding
from hiperhealth.agents.extraction.vision import MedicalVisionExtractor, MedicalVisionExtractorError
...
def test_pipeline_extraction(mock_completion, mock_image):
    ...
    mock_choice.message.content = '{"findings": [{"region_tag": "skin", "clinical_finding": "erythema", "confidence_score": 0.85}], "image_quality_notes": "Good"}'
    mock_response.choices = [mock_choice]
    mock_completion.return_value = mock_response

    pipeline = MedVisionPipeline()
    result = pipeline.analyze_image(mock_image, clinical_context="Patient complains of rash")
...
def test_medical_vision_extractor_flow(mock_analyze_image):
    ...
    mock_analyze_image.return_value = VisionExtractionResult(
        findings=[VisionFinding(region_tag="eye", clinical_finding="clear", confidence_score=0.9)],
...
def test_medical_vision_extractor_input_types(mock_analyze_image, tmp_path):
    ...
    with pytest.raises(MedicalVisionExtractorError, match="Unsupported source type"):
...
def test_medical_vision_extractor_pipeline_error(mock_analyze_image):
    ...
    extractor = MedicalVisionExtractor()
    with pytest.raises(MedicalVisionExtractorError, match="Vision analysis failed: API down"):
